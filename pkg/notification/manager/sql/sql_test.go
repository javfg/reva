...
var _ = Describe("SQL manager for notifications", func() {
	...
	Context("Creating notifications", func() {
		...
		When("updating an existing notification", func() {
			...
			It("should delete old entries in notification recipients", func() {
				var count int
				err = db.QueryRow("SELECT COUNT(*) FROM cbox_notification_recipients WHERE user_name = 'testuser'").Scan(&count)
				Expect(err).ToNot(HaveOccurred())
	...
	Context("Getting notifications", func() {
		...
		When("getting a non-existing notification", func() {
			JustBeforeEach(func() {
				nn, err = mgr.GetNotification("non-existant-ref")
			})

			It("should return a NotificationNotFoundError", func() {
				_, isNotificationNotFoundError := err.(*notification.NotificationNotFoundError)
				Expect(err).To(HaveOccurred())
				Expect(isNotificationNotFoundError).To(BeTrue())
			})
		})
	})
	...
	Context("Deleting notifications", func() {
		...
		When("deleting a non-existing notification", func() {
			JustBeforeEach(func() {
				err = mgr.DeleteNotification("non-existant-ref")

			})

			It("should not change the db and return a NotificationNotFoundError error", func() {
				Expect(err).To(HaveOccurred())
				isNotificationNotFoundError, _ := err.(*notification.NotificationNotFoundError)
				Expect(isNotificationNotFoundError).ToNot(BeNil())
				var count int
				err = db.QueryRow("SELECT COUNT(*) FROM cbox_notifications").Scan(&count)
				Expect(err).ToNot(HaveOccurred())
				Expect(count).To(Equal(1))
