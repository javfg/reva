...
// New returns an instance of the sql notifications manager.
...
func (m *mgr) UpsertNotification(n notification.Notification) error {
	...
	stmt, err = tx.Prepare("REPLACE INTO cbox_notification_recipients (notification_id, user_name) VALUES (?, ?)")
	if err != nil {
		tx.Rollback()
		return err
	}
	...
	for _, userName := range n.Recipients {
		_, err := stmt.Exec(notificationId, userName)
		if err != nil {
			tx.Rollback()
			return err
		}
...
func (m *mgr) GetNotification(ref string) (*notification.Notification, error) {
	query := `
		SELECT n.id, n.ref, n.template_name, nr.user_name
	...
	for rows.Next() {
		var id string
		var userName string
		err := rows.Scan(&id, &n.Ref, &n.TemplateName, &userName)
		if err != nil {
			return nil, err
		}
		n.Recipients = append(n.Recipients, userName)
		count++
	}
	...
	if count == 0 {
		return nil, &notification.NotificationNotFoundError{
			Ref: n.Ref,
		}
	}
...
func (m *mgr) DeleteNotification(ref string) error {
	...
	if rowsAffected == 0 {
		return &notification.NotificationNotFoundError{
			Ref: ref,
		}
	}
