...
type config struct {
	...
	DefaultSender  string `mapstructure:"default_sender" docs:";Default sender when not specified in the trigger."`
}
...
func (e *EmailHandler) Send(sender, recipient, subject, body string) error {
	...
	err := smtp.SendMail(e.conf.SMTPAddress, e.getAuth(), utils.WithDefault(sender, e.conf.DefaultSender), []string{recipient}, msg)
	if err != nil {
		return err
	}
...
func (e *EmailHandler) generateMsg(from, to, subject, body string) []byte {
	...
	headers := []string{
		fmt.Sprintf("From: %s", utils.WithDefault(from, e.conf.DefaultSender)),
		fmt.Sprintf("To: %s", to),
