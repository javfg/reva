...
func getNotificationManager(c *config, l *zerolog.Logger) (notification.Manager, error) {
	...
	return nil, errtypes.NotFound("storage driver not found: " + c.StorageDriver)
}
...
func New(m map[string]interface{}, log *zerolog.Logger) (rserverless.Service, error) {
	...
	if err != nil {
		...
		log.Info().Msgf("notification storage %s initialized (interval: %d, maxSize: %d)", conf.StorageDriver, conf.GroupingInterval, conf.GroupingMaxSize)
	}
...
func (s *svc) Start() {
	s.conf.NatsPrefix = utils.WithDefault(s.conf.NatsPrefix, "reva-notifications")
	...
	err := s.connect()
	if err != nil {
		s.log.Error().Err(err).Msg("error connecting to nats")
	} else {
		s.log.Info().Msg("notification service ready")
	}
}
...
func (s *svc) connect() error {
	nc, err := nats.Connect(s.conf.NatsAddress,
		nats.DisconnectErrHandler(func(_ *nats.Conn, err error) {
			s.log.Error().Err(err).Msgf("disconnected from nats server")
		}),
		nats.ReconnectHandler(func(_ *nats.Conn) {
			s.log.Info().Msgf("reconnected to nats server")
		}),
		nats.ClosedHandler(func(_ *nats.Conn) {
			s.log.Info().Msgf("closed connection to nats server")
		}),
		// nats.Token(s.conf.NatsToken),
	)
	if err != nil {
	...
	if err := s.initNatsStream("trigger", s.handleMsgTrigger); err != nil {
		return err
	}
...
func (s *svc) initNatsStream(name string, handler func(msg *nats.Msg)) error {
	...
	s.js.Subscribe("", func(msg *nats.Msg) { handler(msg) }, nats.Bind(streamName, consumerName))
...
func (s *svc) handleMsgTemplate(msg []byte) {
	...
	name, err, delete := s.templates.Put(msg, s.handlers)
	if err != nil {
...
func (s *svc) handleMsgUnregisterNotification(msg *nats.Msg) {
	...
	if err != nil {
		_, isNotificationNotFoundError := err.(*notification.NotificationNotFoundError)
		if isNotificationNotFoundError {
...
func (s *svc) getAccumulatorForTrigger(tr trigger.Trigger) *accumulator.Accumulator[trigger.Trigger] {
	...
	if !ok || a == nil {
		...
		a.Start(s.notificationSendCallback)
...
func (s *svc) handleMsgTrigger(msg *nats.Msg) {
	...
	if err != nil {
		s.log.Error().Err(err).Msgf("trigger notification template %s not found", notif.TemplateName)
		return
	}

	notif.InitTemplate(*templ)
	tr.InitNotification(*notif)
...
func (s *svc) notificationSendCallback(ts []trigger.Trigger) {
	...
	if len(ts) == 1 {
		...
		s.log.Info().Msgf("sending aggregated notification composed by %d %s templates", tr.TemplateData["_count"], notif.TemplateName)
	}
