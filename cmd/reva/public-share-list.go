...
func publicShareListCommand() *command {
	...
	cmd.Action = func(w ...io.Writer) error {
		...
		if len(w) == 0 {
			...
			t.AppendHeader(table.Row{"#", "Owner.Idp", "Owner.OpaqueId", "ResourceId", "Permissions", "Token", "Expiration", "Created", "Updated", "Description", "NotifyUploads", "NotifyUploadsExtraRecipients"})

			for _, s := range shareRes.Share {
				t.AppendRows([]table.Row{
					{s.Id.OpaqueId, s.Owner.Idp, s.Owner.OpaqueId, s.ResourceId.String(), s.Permissions.String(), s.Token, s.Expiration.String(), time.Unix(int64(s.Ctime.Seconds), 0), time.Unix(int64(s.Mtime.Seconds), 0), s.Description, s.NotifyUploads, s.NotifyUploadsExtraRecipients},
				})
			}
			t.Render()
		} else {
